...
exports.handler = (event, context, callback) => {
    ...
    // 클라이언트 IP 주소 가져오기
    // CloudFront는 X-Forwarded-For 헤더의 첫 번째 IP를 사용
    const clientIP = request.clientIp;
    
    // 환경 변수에서 화이트리스트 가져오기 (콤마로 구분된 CIDR 목록)
    // Lambda@Edge는 환경 변수 크기 제한이 있으므로 콤마로 구분된 문자열로 전달
    const whitelistEnv = process.env.IP_WHITELIST || '';
    const whitelist = whitelistEnv ? whitelistEnv.split(',').map(ip => ip.trim()).filter(ip => ip.length > 0) : [];
