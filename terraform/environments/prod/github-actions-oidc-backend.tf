  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:${data.aws_partition.current.partition}:ecr:${var.aws_region}:${data.aws_caller_identity.current.account_id}:repository/${aws_ecr_repository.bt_portal_backend_prod.name}"
      },
      {
        Effect = "Allow"
        Action = [
      ...
      },
      {
        ...
        Resource = "${aws_s3_bucket.deploy_artifacts.arn}/bt-portal-backend/prod/*"
      },
      {
        Effect = "Allow"
        Action = [
